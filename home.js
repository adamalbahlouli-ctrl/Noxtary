...
.nc-unified {
    ...
    border: 1px solid var(--card-border);
    border-radius: var(--card-radius);
    border-left: 3.5px solid var(--type-color);
    box-shadow: var(--card-shadow);
    backdrop-filter: blur(10px);
    transition: var(--card-transition);
    position: relative;
...
.nc-unified:hover {
    ...
    box-shadow: 0 8px 28px rgba(0,0,0,0.5),
                0 0 16px color-mix(in srgb, var(--type-color) 15%, transparent);
...
@media (max-width: 768px) {
    ...
    .login-btn      { padding: 7px 14px; font-size: 0.7rem; }
    .translate-btn  { padding: 7px 12px; font-size: 0.7rem; }
...
@media (max-width: 480px) {
    .brand-title    { font-size: 2rem; letter-spacing: 2px; }
